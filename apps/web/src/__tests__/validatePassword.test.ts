...
/**
 * 비밀번호 검증 단위 테스트.
 * 명세(`context/sso-api/frontend-auth.md`): 8~19자. 조합(대문자+소문자+숫자+특수기호)은 FE 정책.
 */
describe("validatePassword", () => {
  const LENGTH_ERROR = "비밀번호는 8~19자여야 합니다.";
  const COMBO_ERROR =
    "비밀번호는 대문자, 소문자, 숫자, 특수기호를 각각 1개 이상 포함해야 합니다.";
  ...
  it("대문자가 없으면(소문자+숫자+특수기호) 조합 에러를 반환한다", () => {
    expect(validatePassword("abcdef1!")).toBe(COMBO_ERROR);
  });

  it("소문자가 없으면(대문자+숫자+특수기호) 조합 에러를 반환한다", () => {
    expect(validatePassword("ABCDEF1!")).toBe(COMBO_ERROR);
  });

  it("숫자가 없으면(대문자+소문자+특수기호) 조합 에러를 반환한다", () => {
    expect(validatePassword("Abcdef!@")).toBe(COMBO_ERROR);
  });

  it("특수기호가 없으면(대문자+소문자+숫자) 조합 에러를 반환한다", () => {
    expect(validatePassword("Abcdef12")).toBe(COMBO_ERROR);
  });

  it("소문자만 8자는 조합 에러를 반환한다", () => {
    expect(validatePassword("abcdefgh")).toBe(COMBO_ERROR);
  });

  it("대문자만 8자는 조합 에러를 반환한다", () => {
    expect(validatePassword("ABCDEFGH")).toBe(COMBO_ERROR);
  });
