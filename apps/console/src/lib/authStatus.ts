 ...
 * 401/403으로 실패하면 인증/인가가 필요한 상태로 본다. 그 감지는 React 트리 밖의 전역
 * 핸들러(`queryClient` onError)에서 일어나므로, 모듈 레벨 스토어에 상태를 담아
 * `useSyncExternalStore`로 컴포넌트(`AuthGate`)가 구독한다.
 *
 * 이 스토어를 두는 이유: 401/403일 때 SSO 로그인으로 **자동 전체 페이지 리다이렉트**하는
 * 대신, 본문 위에 "로그인 필요" 안내 화면을 띄우기 위함이다(로그인은 사용자가 화면의
 * 버튼으로 직접 진행 → `redirectToLogin`).
 */

/** `ok`: 정상/미확정, `unauthenticated`: 401, `forbidden`: 403. */
export type AuthStatus = "ok" | "unauthenticated" | "forbidden";
...
/** 403 응답 감지 — 권한 부족 안내가 필요한 상태로 표시. */
export const markForbidden = (): void => setStatus("forbidden");
