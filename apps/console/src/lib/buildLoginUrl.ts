...
/** 로그인 페이지(web)가 읽는 식별/복귀 쿼리. SSO 로그인 페이지가 읽는 키와 정확히 일치시킵니다(소문자 kebab-case). */
export interface LoginQuery {
  /** 콘솔의 client-type(소문자 `web`|`app`). 토큰을 URL 쿼리로 받는 콘솔은 보통 `app`. */
  clientType: string;
  /** SSO에 콘솔을 식별시키는 OAuth client-id. */
  clientId: string;
  /** 로그인 후 되돌아올 콘솔의 절대 경로. */
  returnTo: string;
}

/**
 * @description 로그인 페이지 주소에 `client-type`/`client-id`/`returnTo`를 쿼리로 첨부한 이동 대상 URL을 반환합니다.
 *
